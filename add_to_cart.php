...
if (!is_array($input)) {
    $input = $_POST; // fallback to form-encoded
}
...
$sql = "SELECT id, restaurant_id FROM menu_items WHERE id = ? AND available = 1";
...
if ((int)$menu_item['restaurant_id'] !== $restaurant_id) {
...
$existing_item = mysqli_fetch_assoc(mysqli_stmt_get_result($existing_stmt));
...
if ($existing_item) {
    ...
    if (mysqli_stmt_execute($update_stmt)) {
        ...
        echo json_encode([
            ...
            'cart_count' => (int)$count_result['total']
        ]);
    } else {
